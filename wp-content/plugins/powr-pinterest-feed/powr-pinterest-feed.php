<?php
    ...
    */
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////Create Pinterest Feed widget/////////////////////////////////
    class Powr_Pinterest_Feed extends WP_Widget{
      public $plugin;
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_pinterest_feed',
                             __( 'POWr Pinterest Feed' ),
                             array( 'description' => __( 'Pinterest Feed by POWr.io') )
        );
      ...
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-pinterest-feed' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Pinterest Feed.
        </p>
        <p>
    ...
    function register_powr_pinterest_feed() {
      register_widget( 'Powr_Pinterest_Feed' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_pinterest_feed' );
    //Create short codes for adding plugins anywhere:
    function powr_pinterest_feed_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-pinterest-feed' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-pinterest-feed' label='$label'></div>";
      }else{
      	"<div class='powr-pinterest-feed'></div>";
      }
    }
    add_shortcode( 'powr-pinterest-feed', 'powr_pinterest_feed_shortcode' );
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_pinterest_feed_menu' );
    function powr_pinterest_feed_menu() {
      add_menu_page( 'POWr Pinterest Feed', 'POWr Pinterest Feed', 'manage_options', 'powr-pinterest-feed-settings', 'powr_pinterest_feed_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_pinterest_feed_options() {
      if ( !current_user_can( 'manage_options' ) )  {
        wp_die( __( 'You do not have sufficient permissions to access this page.' ) );
      }
      echo '<iframe id="powr-pinterest-feed" src="https://www.powr.io/wp-create/pinterest-feed" frameborder="0" width="100%" height="600px" style="position:absolute; top:0; left:0;z-index: 100;"></iframe>';
      echo '<script>';
      echo 'var ht = window.innerHeight; if(document.getElementById("wpadminbar")){ht -= document.getElementById("wpadminbar").offsetHeight;}';
      echo 'var iframe = document.getElementById("powr-pinterest-feed"); iframe.style.height = ht+"px"; iframe.height = ht;';
      echo '</script>';
    }
    //Redirecting to landing page when plugin is activated
    register_activation_hook(__FILE__, 'powr_pinterest_feed_plugin_activate');
      add_action('admin_init', 'powr_pinterest_feed_plugin_redirect');

      function powr_pinterest_feed_plugin_activate() {
      add_option('powr_pinterest_feed_plugin_do_activation_redirect', true);
      }

      function powr_pinterest_feed_plugin_redirect() {
      if (get_option('powr_pinterest_feed_plugin_do_activation_redirect', false)) {
          delete_option('powr_pinterest_feed_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-pinterest-feed-settings' );
          }
       }
      }
