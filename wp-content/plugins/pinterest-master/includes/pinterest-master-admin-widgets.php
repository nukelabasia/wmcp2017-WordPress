<?php
function menu_single_pinterest_admin_widgets(){
	if ( is_admin() )
	add_submenu_page( 'pinterest-master', 'Widgets', 'Widgets', 'manage_options', 'pinterest-master-admin-widgets', 'pinterest_master_admin_widgets' );
}

function pinterest_master_admin_widgets(){
$plugin_master_name = constant('PINTEREST_MASTER_NAME');
?>
<div class="wrap">
<h1>Widgets</h1>
<?php
if(!class_exists('pinterest_master_admin_widgets_table')){
	require_once( dirname( __FILE__ ) . '/pinterest-master-admin-widgets-table.php');
}
//Prepare Table of elements
$wp_list_table = new pinterest_master_admin_widgets_table();
//Table of elements
...
if( is_multisite() ) {
add_action( 'admin_menu', 'menu_single_pinterest_admin_widgets' );
}
else {
add_action( 'admin_menu', 'menu_single_pinterest_admin_widgets' );
}
